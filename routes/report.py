...
@reports.route('/reports/add', methods=['POST'])
def add_report():
    data = request.json
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        ...
        (data['report_id'], data['trip_id'], data['generated_by'], data['report_details'], data.get('created_at'))
    )
    conn.commit()
    conn.close()
    return jsonify({"message": "Report added"})

@reports.route('/reports/update', methods=['POST'])
def update_report():
    data = request.json
    conn = get_connection()
    cursor = conn.cursor()
